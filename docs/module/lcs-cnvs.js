      ...
      if (LcsCnvs.settings.image.src) {
        ...
        img.onload = () => {
          if (LcsCnvs.settings.image.width) {
            img.height =
              (img.height * LcsCnvs.settings.image.width) / img.width;
      ...
      LcsCnvs.setCanvas.drawTriangleAfterNewVertex = () => {
        ...
        for (let i = 0; i < LcsCnvs.settings.vertex.nb; i++) {
          const vertex = getRandomVertex([
            ...
            {
              x:
                LcsCnvs.settings.canvas.width - LcsCnvs.settings.canvas.padding,
              y:
                LcsCnvs.settings.canvas.height - LcsCnvs.settings.canvas.padding
            }
          ]);
          if (vertices.size >= 2) {
